...
def main():
    ...
    model_training = ModelTraining(config, data_prep.preprocessor) # type: ignore
    ...
    # Find the best model based on a classification metric (e.g., "F1" instead of "R²")
    best_model_name = max(all_metrics, key=lambda k: all_metrics[k]["F1"])  # Change "R²" to "F1"
